...
function shortcode_recent_posts($atts, $content = null) {
	...
	$output = '<ul class="recent-posts rlist '.$custom_class.'">';
	...
	foreach($latest as $post) {
		...
		//Standard
			...
			if($more_text_single!=""){
				$output .= '<a href="'.get_permalink($post->ID).'" class="link" title="'.get_the_title($post->ID).'">';
...
function shortcode_popular_posts($atts, $content = null) {
	...
	$output = '<ul class="popular_posts rlist">';
	...
	foreach($popular as $post){
		...
		if ( has_post_thumbnail($post->ID) ){
			$output .= '<figure class="featured-thumbnail fleft"><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'">';
			$output .= '<img  src="'.$image.'"/>';
			$output .= '</a></figure>';
		}
		$output .= '<h6><a href="'.get_permalink($post->ID).'" title="'.get_the_title($post->ID).'">';
...
function shortcode_audio($atts, $content = null) {
	...
	if($mp3 !== false){
		...
		_e("The URL you entered is not a .mp3 file.", "theme1918");
	}
...
function shortcode_video($atts, $content = null) {
	...
	if($vimeo !== false){
		...
		_e("You entered a video URL that isn't compatible with this shortcode.", "theme1918");
	}
