...
function simple_social_options(){
	$simple_social_icons = array('twitterwidget','facebookwidget','googlepluswidget','myspace','blogger','stumbleupon','email','technorati','facebook','twitter','delicious','digg','google','yahoo','reddit','rss');
	foreach ($simple_social_icons as $item) {
		$simple_social_pageoptions.='ss_'.$item.',';
	}
	echo '<form method="post" action="options.php">
		<h3>Text shown above icons:</h3>
		<div style="padding:20px;">
			<input type="text" size=50 name="ss_title" value="'.get_option('ss_title','Did you like this? Share it:').'">
		</div>
		<h3>Select Icon Size:</h3>
		<div style="padding:20px;">
			<input type="radio" value="25" name="ss_iconsize" id=ss_25 '.(get_option('ss_iconsize',32)==25?'checked':'unchecked').'> 
			<label for=ss_16>Theme Icons (25x25)</label>&nbsp;&nbsp;&nbsp;
			<input type="radio" value="16" name="ss_iconsize" id=ss_16 '.(get_option('ss_iconsize',32)==16?'checked':'unchecked').'> 
			<label for=ss_16>Small (16x16)</label>&nbsp;&nbsp;&nbsp;
			<input type="radio" '.(get_option('ss_iconsize',32)==32?'checked':'unchecked').' id=ss_32 value="32" name="ss_iconsize"> 
			<label for=ss_32>Large (32x32)</label>
		</div>
	...
	echo '</div><p class="submit"><input type="submit" class="button-primary" value="Save Changes"/>';
