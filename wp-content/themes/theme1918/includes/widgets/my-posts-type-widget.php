...
class MY_PostsTypeWidget extends WP_Widget {
	...
	function MY_PostsTypeWidget() {
		...
		$control_ops = array('width' => 270, 'height' => 150);
			...
			<li class="cat_post_item-<?php echo $posts_counter; ?> clearfix">
				...
				<div class="post-meta-wrapper">
					<?php if ( $instance['date'] ) : ?>
						<span class="post_meta">Written by <?php the_author_posts_link() ?> <time datetime="<?php the_time('Y-m-d\TH:i'); ?>"><?php the_time('D, j F Y'); ?></time></span>
					<?php endif; 
				...
				if ( $instance['more_link'] ) : ?>
					<a href="<?php the_permalink() ?>" class="link <?php if($instance['more_link_class']!="") {echo $instance['more_link_class'];}else{ ?>link<?php } ?>"><?php if($instance['more_link_text']==""){ ?>Read more<?php }else{ ?><?php echo $instance['more_link_text']; ?><?php } ?></a>
				<?php endif; ?>
			</li>
		<?php }
		echo "</ul>\n";
		...
		<div style="width:230px; float:left; padding-right:20px; border-right:1px solid #c7c7c7;">
				...
				<label>
					...
					<select id="<?php echo $this->get_field_id('posttype'); ?>" name="<?php echo $this->get_field_name('posttype'); ?>" class="widefat" style="width:150px;">
						<?php foreach(get_post_types($args,'names') as $key => $post_type) { 
							...
							if ($key=='page' || $key=='revision' || $key=='attachment' || $key=='nav_menu_item' || $key=='optionsframework'){continue;} ?>
			...
			<p>
				<label for="<?php echo $this->get_field_id("container_class"); ?>">
					...
					<input class="widefat" id="<?php echo $this->get_field_id("container_class"); ?>" name="<?php echo $this->get_field_name("container_class"); ?>" type="text" value="<?php echo esc_attr($instance["container_class"]); ?>"> 
					<span style="font-size:11px; color:#999;"><?php _e( '(default: "featured_custom_posts")' ); ?></span>
				</label>
			</p>
			...
			<fieldset style="border:1px solid #F1F1F1; padding:10px 10px 0; margin-bottom:1em;">
				...
				<p>
					<label for="<?php echo $this->get_field_id("excerpt_length"); ?>"><?php _e( 'Excerpt length (words):' ); ?></label>
			...
			<fieldset style="border:1px solid #F1F1F1; padding:10px 10px 0; margin-bottom:1em;">
				...
				<p>
					<label for="<?php echo $this->get_field_id("more_link_text"); ?>">
						<?php _e( 'Link text' ); ?>:
						<input class="widefat" id="<?php echo $this->get_field_id("more_link_text"); ?>" name="<?php echo $this->get_field_name("more_link_text"); ?>" type="text" value="<?php echo esc_attr($instance["more_link_text"]); ?>"> 
						<span style="font-size:11px; color:#999;"><?php _e( '(default: "Read more")' ); ?></span>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("more_link_class"); ?>">
						<?php _e( 'Link class' ); ?>:
						<input class="widefat" id="<?php echo $this->get_field_id("more_link_class"); ?>" name="<?php echo $this->get_field_name("more_link_class"); ?>" type="text" value="<?php echo esc_attr($instance["more_link_class"]); ?>"> 
						<span style="font-size:11px; color:#999;"><?php _e( '(default: "link")' ); ?></span>
					</label>
				</p>
			</fieldset>
				...
				<?php if ( function_exists('the_post_thumbnail') && current_theme_supports("post-thumbnails") ) : ?>
					...
					<p>
						<label for="<?php echo $this->get_field_id("thumb_w"); ?>">
							<?php _e( 'Width: ' ); ?>
							<input class="widefat" style="width:40%;" type="text" id="<?php echo $this->get_field_id("thumb_w"); ?>" name="<?php echo $this->get_field_name("thumb_w"); ?>" value="<?php echo $instance["thumb_w"]; ?>">
						</label>
					</p>
					<p>
						<label for="<?php echo $this->get_field_id("thumb_h"); ?>">
							<?php _e( 'Height: ' ); ?>:
							<input class="widefat" style="width:40%;" type="text" id="<?php echo $this->get_field_id("thumb_h"); ?>" name="<?php echo $this->get_field_name("thumb_h"); ?>" value="<?php echo $instance["thumb_h"]; ?>">
						</label>
					</p>
					<p>
