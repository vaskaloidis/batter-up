...
function theme1918_random($length){
			...
			<div class="grid_gallery_inner">
				...
				if ($attachments) : 
					foreach ($attachments as $attachment) : 
						...
						$image = aq_resize($img_url, 180, 120, true); ?>

						<div class="gallery_item">
							<figure class="featured-thumbnail single-gallery-item">
								<a href="<?php echo $img_url; ?>" class="image-wrap touch-item">
									<img 
...
if ( !function_exists( 'tz_gallery' ) ) {
	function tz_gallery($postid, $imagesize) {
		$random = theme1918_random(10); ?>
		<script type="text/javascript">
			jQuery(function(){
				jQuery('#gallery_post_<?php echo $random ?>').slides({
					effect: 'fade',
					container: 'slides_container_slideshow',
					crossfade: true,
					preload: true,
		...
		<div id="gallery_post_<?php echo $random ?>" class="gallery_post">
			<div class="slides_container_slideshow">
				<?php $args = array(
					'orderby'		 => 'menu_order',
					'order' => 'ASC',
					'post_type'      => 'attachment',
...
function pagination($pages = '', $range = 4){
	...
	if(1 != $pages) {
		...
		if($paged > 1 && $showitems < $pages) echo "<a href='".get_pagenum_link($paged - 1)."'class='prev'>&lsaquo; Prev</a>";
		for ($i=1; $i <= $pages; $i++) {
			if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems ))
				echo ($paged == $i)? "<span class=\"current\">".$i."</span>":"<a href='".get_pagenum_link($i)."' class=\"inactive\">".$i."</a>";
		}
		if ($paged < $pages && $showitems < $pages) echo "<a href=\"".get_pagenum_link($paged + 1)."\" class='next'>Next &rsaquo;</a>"; 
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		<div id="comment-<?php comment_ID(); ?>" class="comment-body">
			<div class="comment-author vcard">
				...
				printf(__('<span class="author">%1$s</span>' ), get_comment_author_link()) ?>
			</div>
